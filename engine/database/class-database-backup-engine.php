...
class DatabaseBackupEngine
{
    ...
    private function query_unbuffered(string $sql): \mysqli_result
    {
        ...
        try {
            // phpcs:ignore WordPress.DB.RestrictedFunctions.mysql_mysqli_query -- $dbh is $wpdb's own mysqli handle (not a separate connection); MYSQLI_USE_RESULT streams rows unbuffered, which $wpdb->query() cannot do, for memory-safe reads of large backup tables. $sql is built via DatabaseSafetyPolicy::quote_identifier() and $wpdb->prepare().
